...
function assertOwner(uid) {
    ...
    if (allowed.length === 0) {
        ...
        throw new HttpsError("failed-precondition", "لم يُضبط ADMIN_UIDS في بيئة Cloud Functions. راجعي التوثيق في .env.example.");
    }
...
export const getOwnerDashboard = onCall(ownerFnOpts, async (request) => {
    ...
    try {
        ...
        const libraryActivationsByBook = new Map();
        ...
        if (librarySnap) {
            ...
            for (const doc of librarySnap.docs) {
                ...
                if (parent)
                    holders.add(parent.id);
                const d = doc.data();
                if (String(d.status ?? "") !== "paid")
                    continue;
